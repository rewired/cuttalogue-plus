...
- Phase 8 has started: the deterministic fast release gate passes all 16 local backend suites and all 21 frontend suites; recovery, migration, security boundaries, known limitations, manual acceptance, and initial release notes are documented. Real-model alignment smoke, large-scene profiling, and recorded manual acceptance remain before the next release merge into `main`.
